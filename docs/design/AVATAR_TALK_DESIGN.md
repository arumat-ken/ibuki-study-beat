...
読み上げソフト(espeak-ng)で作った実音声48秒を通したところ、
**開き 88% / 半開き 8% / 閉じ 3%** となり、口がほぼ開きっぱなしになった。
この声は音量の起伏が小さく、子音の谷がほとんど無いためで、
**口パクの計算が壊れているのではなく、声の性質がそのまま出ている**。

Realtime API の声はもっと起伏があるので改善するはずだが、
**STEP 5 で本番の声を聞きながら `half_on` と `open_on` を必ず見直すこと。**
目安は「開き 50〜65% / 半開き 20〜30% / 閉じ 15〜25%」。
開きが 80% を超えるなら `open_on` を上げ、閉じが 40% を超えるなら下げる。
