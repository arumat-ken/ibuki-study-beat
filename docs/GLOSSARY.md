...
## LINEとMacの連携（AI Orchestra）

この節は、IBUKI STUDY BEAT本体ではなく、Kenさんが別に使っている **AI Orchestra** のLINE連携を説明します。学習アプリの記録が、この経路を通って送信されるわけではありません。

### AI Orchestra（エーアイ・オーケストラ）

**何か**: 複数のAIとMac上の自動処理を、役割分担させる仕組みです。

**この環境では**: iPhoneのLINEを入口にし、MacのPythonプログラム、Codex、Claude、Geminiなどが、調査・定型処理・レビューを分担します。

**誤解しやすい点**: AIが何人もいるように見えても、LINEの受信そのものを担当するのはMac上のWebhookです。AIが直接LINEを常時見張っているわけではありません。

### Webhook（ウェブフック）

**何か**: あるサービスで出来事が起きたとき、別のコンピュータへ自動で知らせる「呼び鈴」です。

**この環境では**: LINEでメッセージやボタン操作があると、LINEがMacの受信プログラムへ知らせます。

**例**: iPhoneで「ステータス」を押す → LINEがWebhookへ通知 → Macが内容を処理 → LINEへ返信、という順です。

**誤解しやすい点**: Webhookが届いたことと、AIの回答が完成してLINEへ届いたことは別の段階です。

### 公開入口

**何か**: インターネット側から、家やMacの中の特定サービスへ入るための限定された入口です。

**この環境では**: LINEがMacのWebhookへ通知できるよう、受信口だけを外部公開します。

**大事な点**: 公開入口は便利ですが、公開するページや機能を必要最小限にする必要があります。Mac全体を公開する意味ではありません。

### Tailscale（テイルスケール）

**何か**: 自分の端末どうしを、暗号化した専用ネットワークのようにつなぐサービスです。

**この環境では**: iPhoneとMacが同じTailscaleネットワークに入り、端末どうしを安全に見つけやすくします。

**誤解しやすい点**: Tailscaleが「接続中」と表示されても、インターネットからLINE Webhookへ入る公開経路まで必ず成功するとは限りません。

### Tailscale Funnel（テイルスケール・ファネル）

**何か**: Tailscaleネットワークの外、つまりインターネットから、Mac内の決めたサービスだけへHTTPSで入れる公開入口です。

**この環境では**: LINEのWebhook通知を、Mac上の受信プログラムへ届けるために使います。

**例**: `LINE → Funnel → Mac内Webhook` の順で通知が届きます。

**誤解しやすい点**: Funnelの不調は、Mac本体やLINEアプリ全体の故障と同じ意味ではありません。公開入口だけで接続が止まることがあります。

### localhost / 127.0.0.1（ローカルホスト）

**何か**: 「このMac自身だけ」を指す特別な宛先です。`127.0.0.1` はその住所のような番号です。

**この環境では**: LINE受信プログラムはMac内部だけで待ち受け、Funnelを通った通信だけを受け取ります。

**大事な点**: 直接インターネットに待受番号を開けるより、公開範囲を小さくできます。

### DNS（ディーエヌエス）

**何か**: 人が読めるWebアドレスを、接続先の番号へ案内する電話帳のような仕組みです。

**この環境では**: LINEがFunnelのURLへ向かう前に、公開URLを接続先へ変換します。

**誤解しやすい点**: Macが正常でも、DNSの反映途中や外部側の経路不良で、LINEから一時的につながらないことがあります。

### 署名検証（しょめいけんしょう）

**何か**: 届いた通知が本当にLINEから来たものか、改ざんされていないかを確かめる本人確認です。

**この環境では**: LINEの署名が正しくない通知は処理せず、エラーとして拒否します。

**大事な点**: URLを知っている第三者が、LINEのふりをして命令を送ることを防ぐ基本の防御です。

### LINE公式Webhookテスト

**何か**: LINE Developersの管理画面から、登録したWebhook URLへLINEが実際に届くか確認する試験です。

**この環境では**: Macの内部状態ではなく、LINE側から見た公開入口の到達性を確認します。

**誤解しやすい点**: HTTP 200の一回の成功だけでは、iPhoneの操作、AI処理、LINE返信までが常に安定している証明にはなりません。

### 自動復旧 / 再適用

**何か**: 監視が異常を見つけたとき、同じ設定をもう一度適用して一時的な通信不良から戻そうとする処理です。

**この環境では**: Funnelの公開入口が届かない場合に、同じ宛先へ再適用する監視があります。

**誤解しやすい点**: 自動復旧で一度戻っても、根本原因が消えたとは限りません。繰り返す場合は、記録を見て公開範囲や経路そのものを見直します。
...
**何か**: 声で話しかけると、キャラクターが声で返事をし、その声に合わせて口が動き、気持ちに合わせて表情や全身の動作が変わる**別のツール**です。パソコン（MacやWindows）で動かすPythonのプログラムです。

**このアプリでは**: IBUKI STUDY BEAT 本体とは**別物**です。本体（`index.html` や `js/`）には一切手を入れません。本体は「外部通信ゼロ」が凍結仕様なのに対し、AVATAR TALK は会話のために常時インターネットへ繋ぐので、混ぜると凍結仕様が壊れるためです。

**具体例**: 設計は `docs/design/AVATAR_TALK_DESIGN.md`、キャラクターの絵の作り方は `docs/design/AVATAR_TALK_GEMINI_BRIEF.md` にあります。2026-09-05時点では**設計だけができていて、まだ作っていません**。

**誤解しやすい点**: 学習記録には触れません。iPhoneでは動きません（パソコン用です）。会話するたびに料金がかかります（1分あたり10円前後）。

### バストアップ

**何か**: 写真やイラストで、**胸のあたりから頭までを写した画角**のことです。全身を写す「全身」と対になる言葉です。

**このアプリでは**: AVATAR TALK のキャラクターは、この2つの画角を使い分けます。会話中はバストアップ（顔が大きいので口の動きが見える）、挨拶や喜びのときだけ数秒だけ全身に切り替えます。

**具体例**: バストアップは1024×1024の正方形で18枚、全身は768×1024の縦長で8枚、合わせて26枚を作ります。

**誤解しやすい点**: 全身のままだと顔が画面の数%しかなく、**口を動かしても見えません**。だから会話用と動作用で分けています。

### ターンアラウンド

**何か**: キャラクターを**全周から等間隔で撮った資料**のことです。正面・斜め45度・真横・斜め後ろ・真後ろ、というように8方向そろえます。

**このアプリでは**: 3Dモデルを作るために要ります。写真は**見えている面だけ**を記録していて、後頭部も、横顔の奥行きも、腕の裏側も写真には存在しません。3Dはそれらを全部必要とするので、全周ぶんの資料を先にそろえます。

**具体例**: `docs/design/reference/avatar3d/` に48枚を置きます。うち32枚がターンアラウンド(8方向×4衣装)です。

**誤解しやすい点**: 会話用の2D画像とは**要求が正反対**です。2Dは背景も光も演出しますが、ターンアラウンドは背景を無地にし、**影を一切作りません**。影は色として焼き付いてしまい、あとから取り除けないためです。同じプロンプトを使い回すと3Dに使えない資料になります。

### 不気味の谷（ぶきみのたに）

**何か**: 人に似せたものが、**よく出来ているほど、わずかな不自然さが強い違和感になる**という現象です。似ていないうちは可愛く見えるのに、そっくりに近づくと急に気持ち悪く感じます。

**このアプリでは**: 実写の人間を3Dで動かすときに問題になります。静止画の実写は自然に見えるのに、**動いた瞬間に違和感が出る**ことがあります。実写に寄せるほどこの谷は深くなります。

**具体例**: grokのコンパニオンがアニメ寄りの絵柄なのは、技術的な手抜きではなく**この谷を避ける正しい判断**です。

**誤解しやすい点**: 作ってから気づいても手遅れです。`AVATAR3D_INTAKE.md` では、全身を作りきる前に**顔だけ試作して親が見る**手順を入れてあります。

### 着せ替え（きせかえ）

**何か**: キャラクターの衣装を差し替える機能です。

**このアプリでは**: **「あとから足せる機能」ではなく、3Dモデルの作り方そのものの決定**です。服と体を1つの塊として作ってしまうと、服の下の体が存在しないため、服を外すと穴が開きます。あとから着せ替えを足すことは**できず、作り直しになります**。

**具体例**: そのため `avatar3d_multiview_prompts.json` には、フィットしたスポーツウェア姿の `base_body` を最初から入れてあります。これは表示には使わず、**体の形を決めるためだけ**の資料です。

**誤解しやすい点**: 「次のステップでやる」と後回しにできる種類の話ではありません。3Dモデルを発注する時点で「服を分けて作る」と条件に含める必要があります。
...
### 開始・終了時刻（かいし・しゅうりょうじこく）

**何か**: 勉強を始めた時刻と終えた時刻です。二つの差が、その学習に使った実績時間になります。

**このアプリでは**: 記録画面で、過去の勉強をあとからまとめて残すときに使えます。日付・科目・学習種別・内容と一緒に、開始と終了を両方入れると実績時間を自動計算します。例: `16:10` から `17:25` は75分です。

**守る条件**: 同じ日の中で、終了は開始より後、最長12時間です。片方だけ・逆順・日またぎは保存できません。時刻を使わない場合は、従来どおり実績時間を手で入れます。

**BPとの関係**: BPは初回保存時の実績時間を上限に一度だけ計算します。記録を編集して終了時刻を後ろへずらしても、BPだけを増やすことはできません。これは、正しい学習記録とポイント交換を両立するための仕組みです。
