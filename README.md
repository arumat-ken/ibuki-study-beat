...
| 設定 | プロフィール、目標・スローガン、受験情報、科目(追加/色/表示/並び替え)、**AI連携設定**、Y軸設定、データ書き出し/読み込み、全消去(二段階確認) |
...
| ファイル | 変数 | 例 |
|---|---|---|
| `js/app.js` | `APP_VERSION` | `'3.3.0'` |
| `sw.js` | `CACHE` | `'isb-v3.3.0'` |
...
コーチ画面の「🤖 AIに聞く」は、**質問文をクリップボードにコピーし、選んだAIアプリを開く**方式です(iOSではWebアプリから他アプリのAIを直接呼べないため)。
