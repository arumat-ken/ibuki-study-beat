# IBUKI STUDY BEAT — Claude Code作業規約
...
4. 対象が `ready` で、自分のWIPが0件であることを確認してから `claim <ID> --actor claude` を実行する。
5. claimの節目を `snapshot` してから実装を始める。

## Claude Codeの境界

- Claude Codeはアプリ本番実装、単体テスト、受入試験、リリース準備を担当する。
- Codexの設計・UIレビュー・仕様整合・独立検証を尊重し、必要な修正は自分の `claude/*` ブランチで行う。
- `main` へ直接pushしない。専用ブランチとPull Requestを使う。
- 未確定仕様を推測で実装しない。必要ならタスクをblockし、親に判断を求める。

## 保護条件

- 対象は大学受験。主役は毎日の学習時間の確保と見える化である。
- `ibukiStudyBeat.v3`、外部通信ゼロ、グラフ中核仕様、黒×ゴールドのBEATスター世界観を壊さない。
- GUIを変更するタスクでは、右上にリリース版・更新日・更新AI・正確なモデル名（不明なら「未記録」）を表示する。運用値はGUIまたはREADMEに既定値と妥当範囲を示す。
- 外部API・監視は重複実行を防ぎ、最終成功時刻・エラー・データ鮮度を分ける。設定は後方互換かつ可能な限り原子的に保存する。
